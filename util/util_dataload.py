...
def set_load_items(config, subjectID=0, movID=0):
...
def get_movID_runID(config, movietitle, runname):
    movIDs = np.where(  np.array(config['movieInfo']['titleNames']) == movietitle  )[0]
    runNo = int(runname.replace('run-', ''))
    # init set
    movID = movIDs[0]
    runID = runNo-1
    # reset if the mov has 2 episodes
    if movietitle == 'bigbangtheory' or movietitle == 'gis':
        if runNo > 2:
            runID = runNo-3
            movID = movIDs[1]
    return movID, runID
